...
function validint
{
 # Validate first field. Then test against min value $2 and/or max value $3
 # if they are supplied. If they are not supplied, skip these tests.
 ...
 if [ "${number%${number#?}}" = "-" ]; then	# is first character a '-' sign?
